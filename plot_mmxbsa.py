...
def get_mmxbsa_stats(gsolv, replicates=5):
    ...
    for num, sys in enumerate(["wt", "w4f", "w5f", "w6f", "w7f"]):  
        ...
        data = np.array([pre_processing(data=np.genfromtxt(f"data/{sys}/v{i:02d}/1us_noion/MM{gsolv.upper()}SA_ENERGY_OUT_INP1.dat", 
...
def plot_19F_mmxbsa_dist(type="gb", mechanics="mm", index=15, replicates=1, ax=None, linewidth=3, space=100):
    """
    ...
    for num, sys in enumerate(["wt", "n33d", "b3d", "nalld"]):
        ...
        data = np.array([pre_processing(data=np.genfromtxt(f"data/{sys}/v{i:02d}/1us/{mechanics.upper()}{type.upper()}SA_ENERGY_OUT.dat", 
                                 skip_header=2, delimiter=","), time_units=500, index=index)[1] for i in range(0, replicates)])
        data = np.reshape(data, -1)

        # get the title and column names
        with open(f"data/wt/v00/1us/{mechanics.upper()}{type.upper()}SA_ENERGY_OUT.dat") as f:
            title = f.readline()
        ...
        #add_patch(ax, 0.15 + 0.2 * num, -0.435, color, sys.upper(), fontsize=17, recspace=-0.01)
...
#plot_19F_mmxbsa_dist("gb", index=15, replicates=1, space=250)
#plt.tight_layout()
#plt.show()
#plt.savefig("figures/total_mmgbsa_v00.png", dpi=300, transparent=True)
...
def plot_19F_mmpbsa_decomp_dist(index=7, replicates=5, ax=None, linewidth=3, space=100, res=121):
    ...
    for num, sys in enumerate(["wt", "n33d", "b3d", "nalld"]):
        ...
        data = np.array([np.genfromtxt(open(f"data/{sys}/v{i:02d}/1us/MMPBSA_DECOMP_ENERGY_OUT.dat", "r").readlines()[0 : 4 + (401*3)], 
...
def multi_xbsa_plot(type="pb", res=121):
    ...
    for num, axis in enumerate(fig.axes):
        ...
        if type == "decomp":
            ...
            plot_19F_mmxbsa_dist(type, "mm", num, 1, axis, 1.5, space=600)

    fig.tight_layout()
    #plt.show()
    fig.savefig(f"figures/all_terms_mmgbsa_v00.png", dpi=300, transparent=True)
